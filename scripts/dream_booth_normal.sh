...
nohup accelerate launch --config_file "$CONFIG_FILE" --main_process_port 24250 train.py \
  output_dir="$OUTPUT_DIR" datasets.dataset_name="/data1/dif/non-finetune-lora/test_case/target" \
  losses="normal" with_prior_preservation=true train_text_encoder=true > "$LOG_FILE" 2>&1 &
