...
def create_or_replace_lora(
    ...
    #     r=rank, # 这个函数没有rank这个东西
    #     lora_alpha=rank, # 是不是要改？
    #     lora_dropout=lora_dropout,
    #     init_lora_weights="gaussian",
    ...
    model.add_adapter(lora_config) # 这里也改了
    
    lora_layers_A = None
    lora_layers_B = None
    if not is_normal:
        lora_layers_A, lora_layers_B = replace_lora_layers(model, attention_types=target_modules)
    return lora_layers_A, lora_layers_B
...
if __name__ == "__main__":
    ...
    pipe = StableDiffusionPipeline.from_pretrained("/data1/dif/sd1-5", torch_dtype=torch.bfloat16).to("cuda")
    unet = pipe.unet
    ...
    ## run the unet
    unet_lora_layersA, unet_lora_layersB = create_or_replace_lora(
        is_normal=False, 
        model=unet, # 函数签名错了，改了 
        r=8, 
        lora_alpha=8, # 两个rank
        lora_dropout=0, 
        init_lora_weights= 'gaussian', 
    ...
    dtype = unet.dtype  # UNet 是 bfloat16，保持一致
    dummy_input = torch.randn(1, 4, 64, 64, device="cuda", dtype=dtype)
    encoder_hidden_states = torch.randn(1, 77, 768, device="cuda", dtype=dtype)
    timesteps = torch.randint(
        0, noise_scheduler.config.num_train_timesteps, (1,), device="cuda", dtype=torch.long
    )
    ...
    for k, v in unet_lora_layersA.items():
        v += torch.randn(v.shape).to("cuda")
    for k, v in unet_lora_layersB.items():
        v += torch.randn(v.shape).to("cuda")
    ...
    # # 2. 测试 create_or_replace_lora 能否加载多个 LoRA。
    # #
    # #    步骤：
    # #        2.1 重新加载一份干净的 pipeline，避免受到上面测试中的权重污染。
    # #        2.2 先加载 LoRA_A，得到输出；保存 LoRA_A 的权重副本，方便与下一次对比。
    # #        2.3 再加载 LoRA_B（这里仅通过 reset 随机种子让权重初始化不同），得到输出。
    # #        2.4 比较两套输出是否不同；若不同，说明第二次加载确实替换了 LoRA；同时对比两次 LoRA 的权重是否发生了改变。
    # #        2.5 若需要，更可在最后再调用一次 create_or_replace_lora 还原为 LoRA_A，并判断输出是否回到 LoRA_A。
    ...
    # ).to("cuda")
    # unet2 = pipe2.unet
    # noise_scheduler2 = pipe2.scheduler

    # dtype2 = unet2.dtype
    # dummy_input2 = torch.randn(1, 4, 64, 64, device="cuda", dtype=dtype2)
    # encoder_hidden_states2 = torch.randn(1, 77, 768, device="cuda", dtype=dtype2)
    # timesteps2 = torch.randint(
    #     0, noise_scheduler2.config.num_train_timesteps, (1,),
    #     device="cuda", dtype=torch.long
    # )

    # # -------------- 2.2 加载第一套 LoRA，并得到输出 --------------
    # torch.manual_seed(1234)
    # unet_lora_layersA_1, unet_lora_layersB_1 = create_or_replace_lora(
    #     is_normal=False,
    #     model=unet2,
    #     r=8,
    #     lora_alpha=8,
    ...
    # # 记录任意一个 LoRA 参数的形状，方便检查
    # sample_key = next(iter(unet_lora_layersA_1))
    # print(f"[LoRA_A] {sample_key} -> {tuple(unet_lora_layersA_1[sample_key].shape)}")

    # with torch.no_grad():
    #     pred_lora_A = unet2(dummy_input2, timesteps2, encoder_hidden_states2, return_dict=False)[0]

    # # clone 一份权重用于之后比较（必须 clone，否则依旧引用同一个张量）
    # snapshot_loraA = {k: v.clone() for k, v in unet_lora_layersA_1.items()}
    # snapshot_loraB = {k: v.clone() for k, v in unet_lora_layersB_1.items()}

    # # -------------- 2.3 再次加载 LoRA（第二套），并得到输出 --------------
    # torch.manual_seed(5678)
    # unet_lora_layersA_2, unet_lora_layersB_2 = create_or_replace_lora(
    #     is_normal=False,
    #     model=unet2,
    #     r=8,
    #     lora_alpha=8,
    ...
    # with torch.no_grad():
    #     pred_lora_B = unet2(dummy_input2, timesteps2, encoder_hidden_states2, return_dict=False)[0]

    # # -------------- 2.4 对比两套输出与权重 --------------
    # # 输出是否发生变化
    # assert not torch.allclose(pred_lora_A, pred_lora_B, atol=1e-5), \
    #     "两次加载后的输出完全一致，说明 LoRA 并没有被正确替换/切换。"

    # # LoRA 权重是否真的变了
    # # （这里只做一个整体的 L1 差异统计）
    # total_diff_A = sum((snapshot_loraA[k] - unet_lora_layersA_2[k]).abs().sum()
    #                    for k in snapshot_loraA)
    # total_diff_B = sum((snapshot_loraB[k] - unet_lora_layersB_2[k]).abs().sum()
    #                    for k in snapshot_loraB)
    # print(f"LoRA_A 权重总差异: {total_diff_A.item():.6f}")
    # print(f"LoRA_B 权重总差异: {total_diff_B.item():.6f}")
    # assert total_diff_A > 0 and total_diff_B > 0, "LoRA 权重没有发生变化，疑似切换失败。"
