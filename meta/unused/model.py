...
if __name__ == "__main__":
    ...
    text_lora_layers_1 = create_or_replace_lora(
        is_normal=False, 
        model=text_encoder, 
        r=8, 
        lora_alpha=8, 
    ...
    print(f'The dummy 2 - dummy 0 is {(model_pred_0-model_pred_2).abs().mean()}')
    print(f'The dummy 1 - dummy 0 is {(model_pred_0-model_pred_1).abs().mean()}')
    ...
    # set_seed(2052)
    unet_lora_layers_2 = create_or_replace_lora(
        is_normal=False, 
        model=unet, 
        r=8, 
        lora_alpha=8, 
    ...
    model_pred_3 = unet(dummy_input, timesteps, encoder_hidden_states, return_dict=False)[0]
    model_pred_4 = unet(dummy_input, timesteps, encoder_hidden_states, return_dict=False)[0]
    
    for k, v in unet_lora_layers_2.items():
        v.data += torch.randn(v.shape).to("cpu")
    model_pred_5 = unet(dummy_input, timesteps, encoder_hidden_states, return_dict=False)[0] 
    assert not torch.allclose(model_pred_2, model_pred_3, atol=1e-5), 'The dummy 2 - dummy 3 is zero'
    assert torch.allclose(model_pred_0, model_pred_3, atol=1e-5), 'The dummy 0 - dummy 3 is non-zero'
    assert torch.allclose(model_pred_4, model_pred_3, atol=1e-5), 'The dummy 4 - dummy 3 is zero'
    assert not torch.allclose(model_pred_5, model_pred_3, atol=1e-5), 'The dummy 5 - dummy 3 is zero'
    print(f'The dummy 0 - dummy 3 is {(model_pred_0-model_pred_3).abs().mean()}')
    print(f'The dummy 2 - dummy 3 is {(model_pred_2-model_pred_3).abs().mean()}')
    print(f'The dummy 5 - dummy 3 is {(model_pred_5-model_pred_3).abs().mean()}')
